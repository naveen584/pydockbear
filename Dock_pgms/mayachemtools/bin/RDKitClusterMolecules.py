...
def ProcessOptions():
    ...
    OptionsInfo["SMILESContainsTitleLine"] = False
    if MiscUtil.CheckFileExt(Options["--infile"], "smi"):
        OptionsInfo["SMILESContainsTitleLine"] = MiscUtil.DoesSMILESFileContainTitleLine(Options["--infile"])
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionNumberValue("-b, --butinaSimilarityCutoff", float(Options["--butinaSimilarityCutoff"]), {">": 0.0, "<=" : 1.0})
    ...
    MiscUtil.ValidateOptionNumberValue("-n, --numClusters", int(Options["--numClusters"]), {">": 0})
