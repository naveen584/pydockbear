...
See also:
    ...
    RDKitConvertFileFormat.py, RDKitPerformMinimization.py
...
def GenerateAndMinimizeMolConformers(Mol, MolCount, Writer):
    ...
    for ConfID in ConfIDs:
        ...
        Energy = GetConformerEnergy(Mol, ConfID)
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionNumberValue("--energyWindow", float(Options["--energyWindow"]), {">": 0.0})
    
    if not re.match("^none$", Options["--embedRMSDCutoff"], re.I):
        MiscUtil.ValidateOptionNumberValue("--embedRMSDCutoff", float(Options["--embedRMSDCutoff"]), {">": 0.0})
    
    if not re.match("^none$", Options["--energyRMSDCutoff"], re.I):
        MiscUtil.ValidateOptionNumberValue("--energyRMSDCutoff", float(Options["--energyRMSDCutoff"]), {">": 0.0})
    ...
    if not re.match("^auto$", Options["--maxConfs"], re.I):
        MiscUtil.ValidateOptionNumberValue("--maxConfs", int(Options["--maxConfs"]), {">": 0})
    
    MiscUtil.ValidateOptionNumberValue("--maxIters", int(Options["--maxIters"]), {">": 0})
